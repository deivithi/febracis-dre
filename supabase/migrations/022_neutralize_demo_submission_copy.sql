-- U06: notas neutras na UI + fn_admin_create_demo_submission sem marcas legadas nas colunas de notas.

begin;

update public.submissions
set notes = ''
where btrim(coalesce(notes, '')) in (
  'Submissao demo gerada automaticamente.',
  E'Submiss\u00e3o demo gerada automaticamente.'
);

update public.events
set notes = ''
where btrim(coalesce(notes, '')) = 'Evento de demonstracao gerado automaticamente.';
...
begin
  ...
  insert into public.events (
    ...
    p_event_date,
    'executed',
    'Evento CIS registrado pela preparacao administrativa.'
  )
  ...
  if p_status in ('under_review', 'pending_adjustment', 'approved') then
    perform public.fn_set_submission_status(
      v_submission_id,
      'under_review',
      'Triagem da controladoria em curso.'
    );
  end if;
  ...
  if p_status = 'pending_adjustment' then
    perform public.fn_set_submission_status(
      v_submission_id,
      'pending_adjustment',
      'Controladoria solicitou ajustes.'
    );
  end if;
  ...
  if p_status = 'approved' then
    insert into public.submission_approvals (
      ...
      auth.uid(),
      'approved',
      coalesce(p_approval_note, 'Submissao aprovada pela controladoria.')
    );
