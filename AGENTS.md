...
### Persona canónica do Agente DRE

Identidade, frase oficial e proibição de métricas voláteis inventadas: [`docs/agente-persona-febracis.md`](docs/agente-persona-febracis.md) + doutrina em código (`PROMPT_INSTITUCIONAL_FEBRACIS_LINES` em `src/features/submissions/dreAgentContext.ts`). TTL persona/ISA e flags servidor: `.env.example`.
...
- `npm run dev`, `npm run build`, `npm run lint`, `npm run test` (inclui `tests/integration/**`; eval live opcional **`DRE_AGENT_LIVE_EVAL=1`** — [`tests/integration/README-dre-agent-live-evals.md`](./tests/integration/README-dre-agent-live-evals.md)), `npm run test:coverage`, `npm run test:e2e` (defina `E2E=0` para ignorar E2E), `npm run smoke:prod`, `npm run smoke:staging` (imprime ponteiro para [`tests/README-staging-smoke.md`](./tests/README-staging-smoke.md)), scripts `validate:*` quando aplicável.
