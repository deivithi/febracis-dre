...
**Pacote de preparação gerencial (smoke + bullets A.4 + grelha de ensaio):** [briefing-gerente-preparacao.md](./briefing-gerente-preparacao.md).
...
Grelha de ensaio espelhada com timestamps e smoke: [briefing-gerente-preparacao.md](./briefing-gerente-preparacao.md) §“Ensaio do green path”. Marcar **[x]** aqui após **ensaio humano** em produção.
...
| # | Verificação | Como | OK |
|---|-------------|------|-----|
| 1 | Alias canónico | `https://febracis-dre.vercel.app` (team `deivithis-projects`) | [x] |
| 2 | Bundle com Supabase canónico | `SMOKE_PROD_URL=https://febracis-dre.vercel.app npm run smoke:prod` (opc. `SMOKE_STRICT=1`) | [x] |
...
- **Última execução documentada (08/05/2026 BRT, agente):** `SMOKE_STRICT=1 node scripts/smoke-prod-bundle.mjs` com `SMOKE_PROD_URL=https://febracis-dre.vercel.app` — **exit 0**; `htmlOk: true`; `hasRoot: true`; `hasUrl: true`; `hasPub: true`; `hasProjectRefInBundle: true`; `scripts_checked: 4`; detalhe em [briefing-gerente-preparacao.md](./briefing-gerente-preparacao.md).

---

## A.4 Risco reputacional IA (mensagem ao dono)

Versão **executiva em três bullets** (slides/e-mail): [briefing-gerente-preparacao.md](./briefing-gerente-preparacao.md) §“A.4”.
