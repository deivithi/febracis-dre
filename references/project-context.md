...
Última revisão documental: 08/05/2026 BRT — KPI trio Submissões/Assistente + deploy `dpl_nQLyZeEzjoLMjGKbuBs7cCqEqvg6`. Validar sempre contra Vercel/GitHub/Supabase antes de mudar papéis, RLS ou deploy.
...
- **Produção READY atual (`vercel inspect` 08/05/2026 BRT, commit `fdc2242`):** `dpl_nQLyZeEzjoLMjGKbuBs7cCqEqvg6` (URL `https://febracis-f4l44ywr4-deivithis-projects.vercel.app`), alias **`https://febracis-dre.vercel.app`**, região build `iad1`, estado **Ready** — faixa KPI em **Submissões** e **Assistente**: três cartões lado a lado no desktop (`escopo da rede` + **preenchimento da grelha** derivado de `draftValidation` + **EBITDA 2 da prévia**); ver [`SubmissionKpiSection.tsx`](../src/features/submissions/components/SubmissionKpiSection.tsx) e `.submissions-kpi-primary` em [`SubmissionsPage.css`](../src/features/submissions/SubmissionsPage.css).
