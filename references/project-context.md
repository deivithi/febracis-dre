...
Última revisão documental: 08/05/2026 BRT — Dashboard executivo (`/app/dashboard`): refactor P0+P1 — **Visão rede** (rótulo do escopo `holding`), KPI Holding únicos alinhados ao filtro (`holdingDerivations.ts` + estado no `DashboardPage`), hero compacto (um **H1**), secundário “Atualizar leitura” com `invalidateQueries ['dashboard']`, tabela cockpit com scroll + cabeçalho fixo, matriz de escopo [`references/dashboard-scope-matrix.md`](./dashboard-scope-matrix.md). **Deploy produção:** `dpl_JA9qevtJ9eWra1iDsQHVbAKGVMyF` (ver **Raiz e URLs**).
