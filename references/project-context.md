...
Última revisão documental: 08/05/2026 BRT — Dashboard executivo (`/app/dashboard`): refactor P0+P1 — **Visão rede** (rótulo do escopo `holding`), KPI Holding únicos alinhados ao filtro (`holdingDerivations.ts` + estado no `DashboardPage`), hero compacto (um **H1**), secundário “Atualizar leitura” com `invalidateQueries ['dashboard']`, tabela cockpit com scroll + cabeçalho fixo, matriz de escopo [`references/dashboard-scope-matrix.md`](./dashboard-scope-matrix.md). **Registo de deploy:** primeira entrada “Produção READY” em **Raiz e URLs** (atualizar `dpl_*` com `npx vercel inspect` após `npx vercel --prod --yes`).
...
- **Produção READY (`vercel inspect` pós-merge deste bloco Dashboard, 08/05/2026 BRT — substituir `dpl_*` pelo output da CLI quando disponível):** commit em `main` com cockpit Holding + KPIs unificados, label **Visão rede** (`access.ts`), `holdingDerivations.ts`, matriz [`references/dashboard-scope-matrix.md`](./dashboard-scope-matrix.md); alias esperado **`https://febracis-dre.vercel.app`**.
...
- **Painel executivo** (`/app/dashboard`): **um H1** (“Painel executivo”); barra contextual sem segundo título competitivo; escopo rede exibido como **Visão rede**; KPIs em modo **holding** seguem filtros competência/regional/unidade (`deriveHoldingView` → `DashboardPage`). Painel lateral sem `Sparkles`/`DatabaseZap`; “Atualizar leitura” chama `queryClient.invalidateQueries({ queryKey: ['dashboard'] })` quando o utilizador não abre Workflow. Tabela do radar com `.table-shell--scroll`. Demais escopos: KPIs franchise/regional/controladoria inalterados em origem (`buildFranchiseKpis` / `buildRegionalKpis` / `buildNetworkKpis`). Matriz UX: [`references/dashboard-scope-matrix.md`](./dashboard-scope-matrix.md).
