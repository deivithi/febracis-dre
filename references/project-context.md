...
- **Shell visual (app autenticada):** tokens `--shell-*` em [`tokens.css`](../src/styles/tokens.css); backdrop (`::before` / `::after`) em [`AppLayout.css`](../src/layouts/app/AppLayout.css); sidebar, header translúcido e lavagem da área principal em [`layout.css`](../src/styles/components/layout.css). Login partilha `--shell-login-mesh` / `--shell-bg-base` ([`LoginPage.css`](../src/features/auth/LoginPage.css)). O mesh “canvas” do chat (`--chat-canvas-mesh`) mantém-se **localizado** ao thread/compositor — não cobre o dashboard inteiro. Critérios: [`references/ux-excellence-roadmap.md`](./ux-excellence-roadmap.md) §1.2.
