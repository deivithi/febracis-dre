...
Última revisão documental: 08/05/2026 BRT — KPI “Preenchimento da grelha” com texto **X de Y linhas obrigatórias**, barra e hint de envio (alinhado ao rail) + deploy `dpl_Ck1NYFjrEMUbh19UGiabqJGTpwUm`. Validar sempre contra Vercel/GitHub/Supabase antes de mudar papéis, RLS ou deploy.
...
- **Produção READY atual (`vercel inspect` 08/05/2026 BRT, commit `1284d00`):** `dpl_Ck1NYFjrEMUbh19UGiabqJGTpwUm` (URL `https://febracis-hpkkxljga-deivithis-projects.vercel.app`), alias **`https://febracis-dre.vercel.app`**, região build `iad1`, estado **Ready** — faixa KPI: cartão **preenchimento da grelha** com contador **X de Y linhas obrigatórias** na linha de cabeçalho, barra de progresso (mesmo tratamento visual que `draft-progress` no rail) e texto *Faltam N linha(s) com valor para liberar o envio*; ver [`SubmissionKpiSection.tsx`](../src/features/submissions/components/SubmissionKpiSection.tsx), `.draft-progress--kpi-card` em [`SubmissionsPage.css`](../src/features/submissions/SubmissionsPage.css).
...
- **Painel executivo** (`/app/dashboard`): copy reformulada para linguagem de dono de empresa — "O resultado da rede em uma página", KPI cards com nomes legíveis ("Receita Bruta de Vendas", "Margem de contribuição 1", "EBITDA 2"), subtítulos "do faturamento bruto" / "de margem média" no lugar de "% RBV". Empty states e textos de erro em português natural ("Quando a unidade enviar a primeira DRE…", "Não conseguimos identificar o seu perfil…").
