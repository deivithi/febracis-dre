...
- **Produção READY atual (2026-05-08, recuperação via CLI no team correto):** `dpl_9GRYX3oQrPqNtqrSHbXJM4RBvWyw`, alias `https://febracis-dre.vercel.app`; origem código = clone local sincronizado com `main` no momento do deploy (`npx vercel pull --yes` quando faltar link local, depois `npm run build` + `npx vercel --prod --yes`).
