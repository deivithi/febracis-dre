...
O ícone da **aba do browser** e o **apple-touch-icon** (iOS) são ficheiros estáticos em `public/` (`favicon-16.png`, `favicon-32.png`, `apple-touch-icon.png`). A cor dourada replica o filtro da marca (rasterizado em `npm run favicons`). Os ícones **16/32** mostram **só a águia** (recorte + trim + preenchimento quadrado); o **180×180** usa o lockup completo. O **fundo** na geração é **transparente por defeito na aba** (16/32) e **creme `#FBF6EC` por defeito no apple-touch** — overrides em **Favicons** no `README.md`.
