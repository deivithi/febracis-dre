...
|--------|----------|--------|----------------|
| **2.2-agent8** | **17/05/2026** | **Agente DRE — modo Dúvidas: explicação substantiva (anti-loop)** | `runLocalAssistantTurn` em `explain_only`: se `classifyDreUserIntent` → `off_topic` mas existe linha guiada em evidência, a resposta **começa** por `getFieldGuide` (`help` + `example`) em vez de só templates `explain_off_topic` + «quer detalhar?»; continuações (`isGuidedFlowContinuationMessage`) com `helpLine` devolvem `guide` após o ack curto. Nova pool `explain_off_topic_hint_after_substance`; `dreHints` alargado (ex.: `cis`, `febracis`, `detal`, `preenc`, `custo`, `logist`); alinhamento de gatilhos `detal`/`esclarec` ao ramo `full` para perguntas de campo. Testes: `tests/unit/dreAssistant.continuation.test.ts` (ex.: `variable_logistics`). **Deploy produção:** commit **`a63b319`**, **`dpl_Aj3CsWhLv72MptzQ1dDLSGmcZ2MW`**, alias `https://febracis-dre.vercel.app`. |
| **2.2-agent7** | **17/05/2026** | **Agente DRE — voz CIS e bordões (Yes / caraca) sem personificar PV** | Tom inspirado na linha pública institucional (Método CIS / autocontenção disciplinada — **sem** falar em nome de Paulo Vieira). `dreAgentContext.ts`: `PROMPT_INSTITUCIONAL_FEBRACIS_LINES`; `api/dre-agent.ts`: secção de voz + uso raro de *Yes.* / *Isso aí.*, *caraca* só em registo positivo e raro, proibição do trio repetido «Yes Yes Yes»; `dreAssistant.ts`: pools `ASSISTANT_FALLBACK_COPY_VARIANTS` com *Yes.* e *Caraca* em anchors/continuação. **Deploy produção:** commit **`6085853`**, **`dpl_DTf6cFjrQLpFyVYYs8gur13U8HV9`**, alias `https://febracis-dre.vercel.app`. |
| **2.2-agent6** | **17/05/2026** | **Agente DRE — hub: progresso, avisos de gravação e memória «onde paramos»** | Motor local: `isGuidedFlowStatusQuestionMessage` tratado **antes** de `isGuidedFlowContinuationMessage` (mensagens tipo «Vamos continuar? Onde paramos?» devolvem `composeGuidedWhereAmIAnswer` com fase/campos). Hub `/app/assistant`: faixa compacta de progresso (fase, %, «Próximo no roteiro»); faixa explícita quando workflow bloqueado vs quando o perfil só orienta com tab **Começar a DRE**. Teste Vitest mensagem combinada. **Deploy produção (paridade Git ⟷ bundle):** commit **`9889616`**, **`dpl_Dyw4QeubRSk8wJ2MAdczdbRfbavf`**, alias `https://febracis-dre.vercel.app` (feature em `8608840`, docs de fecho em `9889616`). |
| **2.2-agent5** | **16/05/2026** | **Agente DRE — etapa em NL, faixa UX e copy multi-variante** | Perguntas naturais de etapa («em qual etapa», «onde estamos») não disparam off-topic curto: `classifyDreUserIntent` + `isGuidedFlowStatusQuestionMessage`; resposta determinística `composeGuidedWhereAmIAnswer` / `runGuidedFlowStatusQuestionTurn`; API `api/dre-agent.ts` ramo **`guided_where_am_i_nl`** (antes de `off_topic`); `skippedLineCodes` nas chamadas `runLocalAssistantTurn` em continuação e off-topic; checkpoint `dre_question`. UI: `bubbleLooksLikeGuidedWhereAmIAnswer` e última mensagem do utilizador tipo etapa suprimem faixa de realinho quando o checkpoint ficou `off_topic` obsoleto. Export **`ASSISTANT_FALLBACK_COPY_VARIANTS`** com pools 2–3 por intent (`explain_continue_anchor_*`, `explain_off_topic_realign_*`, `full_numeric_proposal`, etc.). Testes: `tests/unit/dreAssistant.continuation.test.ts`, `tests/unit/fallback-copy-variants.test.ts`. **Deploy produção:** commit **`3078208`**, **`dpl_E4f8LF2xiPRaRmxdYGNoG7z6ovbd`**, alias `https://febracis-dre.vercel.app`. |
| **2.2-agent4** | **16/05/2026** | **Agente DRE — Hub assistant minimal + continuação guiada** | **`/app/assistant`** reduz chrome da página aos segmentos **Dúvidas** / **Começar a DRE** (query `tab=duvidas`) + thread de chat; `AppLayout` omite breadcrumb e «Escopo ativo» nesta rota; `AssistantDock` + `DreAssistantPanel` com `minimalHubLayout` (sem rail guiado/teclado no hub; preserva composer, HITL e faixas mínimas de persistência/submissão quando necessário). Continuação: frases curtas («vamos continuar?», ok, próximo passo) avançam roteiro antes de `off_topic`; `api/dre-agent.ts` ramo **`continue_guided`**; `dreAssistant.ts` com `isGuidedFlowContinuationMessage`, `bubbleCoversAssistantStepHint`, `pickUiRealignBannerHint` — faixa laranja só quando o passo ainda não está na última bolha; copy de realinho mais leve. Testes: `tests/unit/dreAssistant.continuation.test.ts`; E2E `assistant-guided.spec.ts` ajustado ao hub. **Deploy produção:** commit **`7fa1906`**, **`dpl_5HHLJYoge2b4hTuUnEviNPvGXzoi`**, alias `https://febracis-dre.vercel.app`. |
